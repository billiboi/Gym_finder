...
9. `20260715_001_claim_requests_updated_at_repair.sql` (idempotent repair: re-asserts the migration 7 columns, `updated_at` included — applied to staging and production 2026-07-15, see the resolved-gap note below)
...
**Resolved gap #2 (found and fixed 2026-07-15):** `/admin/richieste` and `/admin/qualita` started 500ing again in production on `column claim_requests.updated_at does not exist` — the exact same symptom as the gap above, despite that fix having been logged as verified. Root cause: almost certainly the same SQL-editor paste issue recurring, silently dropping part of the `alter table` statement on a later manual apply. Fixed with `20260715_001_claim_requests_updated_at_repair.sql` (re-asserts the full column block, `timestamptz` throughout). Applied manually via the SQL editor to staging first, then production, same day. Verified both times by replicating the exact query `readClaimRequestsListFromSupabase` runs (same `select`/`order`/`limit`) directly against each project — 200 OK on both, `public.gyms` row count unchanged on production (692 before and after).
