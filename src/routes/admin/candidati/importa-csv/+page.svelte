    ...
    <div class="flex flex-wrap items-end justify-between gap-4">
      <div>
        ...
        <p class="mt-2 max-w-3xl text-sm text-slate-600">
          Carica un CSV, controlla intestazioni, mapping, anteprima e duplicati. Le righe che corrispondono a una scheda
          esistente la aggiornano direttamente; le righe nuove vanno in coda di revisione candidati. Nessun dato viene
          scritto prima del dry-run e della conferma finale.
        </p>
      </div>
      ...
      <section class="mt-5 rounded-2xl border border-slate-200 bg-white p-5">
        ...
        <div class="mt-4 grid gap-3 md:grid-cols-3">
          <label class="rounded-2xl border border-slate-200 p-4 text-sm">
            ...
            <span class="mt-2 block text-slate-600">Scelta consigliata: protegge i dati già revisionati a mano. Le righe nuove vanno comunque in coda candidati.</span>
          </label>
          <label class="rounded-2xl border border-slate-200 p-4 text-sm">
            <input type="radio" bind:group={importMode} value="create-only" />
            <span class="ml-2 font-bold text-slate-900">Solo nuove schede</span>
            <span class="mt-2 block text-slate-600">Le righe che corrispondono a una scheda esistente vengono saltate; solo le nuove vanno in coda candidati.</span>
          </label>
    ...
    {#if form?.dryRun}
      <section class="mt-5 rounded-2xl border border-emerald-200 bg-emerald-50 p-5">
        ...
        <div class="mt-3 grid gap-3 text-sm text-emerald-900 sm:grid-cols-2 lg:grid-cols-4">
          <p class="rounded-xl bg-white/75 px-3 py-2"><strong>Schede attuali:</strong> {form.report.existingCount}</p>
          <p class="rounded-xl bg-white/75 px-3 py-2"><strong>Aggiornate direttamente:</strong> {form.report.matchedCount}</p>
          <p class="rounded-xl bg-white/75 px-3 py-2"><strong>Nuove in coda candidati:</strong> {form.report.candidateCount}</p>
          <p class="rounded-xl bg-white/75 px-3 py-2"><strong>Saltate:</strong> {form.report.skipCount}</p>
        </div>
        <p class="mt-3 text-sm text-emerald-900">
          Modalità: <strong>{form.mode}</strong>. Le righe nuove non vengono pubblicate subito: restano in
          <a href="/admin/candidati" class="underline">coda di revisione</a> come i candidati dello scraper OSM.
        </p>

        {#if form.report.warnings?.length}
          <div class="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-4">
            <h3 class="text-sm font-bold text-amber-950">Avvisi non bloccanti</h3>
            <p class="mt-1 text-sm text-amber-900">
              L’import può proseguire, ma queste righe meritano controllo prima della conferma finale.
            </p>
            <ul class="mt-3 grid gap-2 text-sm text-amber-900">
              {#each form.report.warnings as warning}
                <li class="rounded-xl bg-white/70 px-3 py-2">
                  Riga {warning.line} · <strong>{warning.name}</strong>: {warning.message}
                </li>
              {/each}
            </ul>
          </div>
    ...
    {#if form?.imported}
      <section class="mt-5 rounded-2xl border border-emerald-200 bg-emerald-50 p-5">
        ...
        <p class="mt-2 text-sm text-emerald-900">
          Aggiornate direttamente: <strong>{form.report.matchedCount}</strong>.
          Nuove in coda candidati: <strong>{form.report.candidateCount}</strong>.
          Saltate: <strong>{form.report.skipCount}</strong>.
        </p>
        <p class="mt-3 text-sm text-emerald-900">
          Le righe nuove sono in attesa in <a href="/admin/candidati" class="underline">coda di revisione</a>.
        </p>
