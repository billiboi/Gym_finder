...
import { canWriteGymCandidates, createGymCandidates } from '$lib/server/gym-candidates-store';
...
// Stable id for gym_candidates' (source, source_id) unique constraint: hashing
// the same identity key used for exact-match dedup means re-importing the same
// CSV updates the same pending candidate instead of creating a duplicate.
function candidateSourceId(gym) {
  return createHash('sha256').update(normalizeKey([gym.name, gym.city, gym.address])).digest('hex').slice(0, 32);
}

function gymToCandidateRow(gym, line) {
  const flags = qualityWarnings(gym, line).map((warning) => ({
    type: 'quality',
    severity: 'warning',
  ...
  return {
    nome: gym.name,
    indirizzo: gym.address || null,
    citta: gym.city || null,
    provincia: gym.provincia || null,
    regione: gym.regione || null,
    telefono: gym.phone || null,
    email: gym.email || null,
    sito: gym.website || null,
    discipline: gym.discipline,
    disciplines: gym.disciplines,
    orari: gym.hours_info,
    latitude: gym.latitude,
    longitude: gym.longitude,
    descrizione: gym.description || null,
    source: 'csv_import',
    source_id: candidateSourceId(gym),
    validation_flags: flags
  };
}

// Righe che corrispondono a una scheda già pubblicata continuano ad
// aggiornarla direttamente (percorso già considerato sicuro); righe senza
// corrispondenza vanno in coda gym_candidates per revisione manuale, come
// per lo scraper OSM — invece di scrivere su gyms senza controllo.
function buildImportPlan(existingGyms, imported, mode, baseWarnings = []) {
  ...
  for (const item of imported) {
    ...
    if (Number.isInteger(matchIndex)) {
      if (mode === 'create-only') {
        skipped.push({ line: item.line, name: item.gym.name, reason: 'corrisponde a una scheda esistente' });
        continue;
      }
      updatedGyms.push(mergeGym(existingGyms[matchIndex], item.gym, mode));
      matches.push({ line: item.line, name: item.gym.name });
      warnings.push({
        line: item.line,
        name: item.gym.name,
        message: 'Corrisponde a una scheda esistente: verrà aggiornata direttamente.'
      });
      continue;
    }

    newCandidates.push(gymToCandidateRow(item.gym, item.line));
    candidates.push({ line: item.line, name: item.gym.name });
  }
  ...
  return {
    ...
    newCandidates,
    report: {
      existingCount: existingGyms.length,
...
function writeGuardFailure() {
  return fail(503, {
    error:
      'Import bloccato: questa area admin deve scrivere su Supabase (schede e coda candidati), ma SUPABASE_SERVICE_ROLE_KEY non è disponibile nell\'ambiente corrente.'
  });
}

export const actions = {
  dryRun: async ({ request }) => {
    if (!canWriteSupabase() || !canWriteGymCandidates()) return writeGuardFailure();
  ...
  confirmImport: async ({ request }) => {
    ...
    const { updatedGyms, newCandidates, report } = buildImportPlan(gyms, parsed.imported, parsed.mode, parsed.warnings);

    try {
      const backup = await backupGyms(gyms);
      if (updatedGyms.length) await writeGymRecords(updatedGyms);
      if (newCandidates.length) await createGymCandidates(newCandidates);
      return {
        imported: true,
        report,
        backup
