<script>
  ...
  const MAP_AUTO_LOAD_CAP = 200;
  const MAP_AUTO_LOAD_MAX_REQUESTS = 8;
  ...
  async function ensureGymsForMap() {
    let requests = 0;
    mapDataLoading = true;
    try {
      while (remoteHasMoreGyms && gyms.length < MAP_AUTO_LOAD_CAP && requests < MAP_AUTO_LOAD_MAX_REQUESTS) {
        await loadGyms();
        requests += 1;
      }
    } finally {
  ...
  onMount(() => {
    ...
    if (window.matchMedia('(min-width: 1024px)').matches) void initMap();
  });
...
<div class="min-h-screen w-full sc-page relative">
  ...
  <section id="mappa-palestre" class:hidden={resultsView !== 'map'} class="overflow-hidden rounded-3xl border border-white/70 bg-white/85 shadow-lg sc-panel sc-map lg:sticky lg:top-5 lg:block" aria-label="Mappa delle palestre filtrate">
    <div class="border-b border-slate-200 px-4 py-4 sm:px-5">
      <div class="flex flex-wrap items-start justify-between gap-3">
        ...
        <div class="flex flex-wrap items-center gap-2">
          <button type="button" class="rounded-2xl sc-map-chip px-3 py-2 text-xs font-semibold transition hover:bg-white lg:hidden" on:click={showResultsList}>
    ...
    <div class="relative">
      ...
      {#if isBootstrapping || mapDataLoading}
      ...
      <div class="border-t border-slate-200 bg-white/92 p-4 sm:p-5">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div class="flex flex-wrap gap-2 sm:justify-end">
            <button type="button" class="inline-flex min-h-[2.6rem] items-center justify-center rounded-xl px-4 text-sm font-bold transition sc-button-ghost lg:hidden" on:click={showResultsList}>
              Vedi nella lista
            </button>
  ...
  <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-3" aria-live="polite" aria-busy={isBootstrapping}>
    ...
    {:else}
      {#each visibleGyms as gym, i}
        ...
        <article
          id={`gym-${gym.id}`}
          class={`group flex h-full flex-col overflow-hidden rounded-2xl border bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl sc-card sc-gym-card ${activeGymId === String(gym.id) ? 'sc-gym-card--active' : ''}`}
