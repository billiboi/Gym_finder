...
function filterGyms(gyms, { q, discipline, openState, userLat, userLng, radiusKm }) {
  ...
  if (discipline) {
    ...
    out = out.filter((gym) => {
      ...
      return list.map((d) => d.toLowerCase()).includes(wanted);
    });
  }
  ...
  if (q) {
    ...
    out = out.filter((gym) => {
      const disciplines = Array.isArray(gym.disciplines) ? gym.disciplines.join(' | ') : String(gym.discipline || '');
      return [gym.name, gym.address, gym.city, disciplines].some((field) =>
